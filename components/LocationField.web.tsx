...
export default function LocationField({ label, value, onChange, containerClassName }: LocationFieldProps) {
  ...
  const useCurrentLocation = () => {
    if (!('geolocation' in navigator)) {
      setError('Your browser does not support location — search for a city below instead.');
      return;
    }
    setIsLocating(true);
    setError(null);
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          setError('Failed to look up your location — search for a city below instead.');
        } finally {
          setIsLocating(false);
        }
      },
