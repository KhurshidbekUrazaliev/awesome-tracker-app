...
export default function Input({
  ...
  const borderColor = error ? 'border-red-500' : isFocused ? 'border-primary-600' : 'border-gray-300';

  return (
    <View className={containerClassName}>
      {label && <Text className="text-sm font-medium text-gray-700 mb-1">{label}</Text>}
      
      <View className={`flex-row items-center border ${borderColor} rounded-lg px-3 bg-white`}>
        ...
        {secureTextEntry && (
          <TouchableOpacity onPress={() => setIsSecure(!isSecure)} className="ml-2">
            <Text className="text-primary-600">{isSecure ? '👁️' : '👁️‍🗨️'}</Text>
          </TouchableOpacity>
        )}
