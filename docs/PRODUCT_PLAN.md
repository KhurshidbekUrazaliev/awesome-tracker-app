...
### Stage 4 — Rental listings (payments, part 1) — 🔶 Built, verified except payout capture
...
- **Not verified**: the final payout step (`transfers.create()` to the owner's connected account + deposit refund) — confirmed via a real test call that it correctly fails with `insufficient_capabilities_for_transfer` until a connected account finishes bank-account linking. That one onboarding step couldn't be completed in this session because Stripe's bank-details form for test accounts renders in a cross-origin iframe that doesn't accept automated browser input at all (tried direct click, typed input, and tab-navigation — none registered). A real owner completing onboarding by hand in their own browser will not hit this; it only blocked automated end-to-end testing of the last step.
