...
### Stage 5 — Auction listings (payments, part 2) — ✅ Built and fully verified
...
- ✅ **End-to-end verified with real Stripe test-mode transactions (2026-09-06)**: created an auction, bid from two accounts, confirmed lower bids reject and the owner can't bid on their own listing, confirmed `POST /:id/interest` correctly redirects auctions to the bidding flow, let the deadline pass and confirmed the lazy on-read close correctly produced a `pending` listing with a synthetic accepted interest for the highest bidder, confirmed the loser is blocked from the checkout URL and the owner is blocked from completing before payment, then completed a real hosted Stripe Checkout payment with a test card and confirmed the webhook captured it and triggered a real `stripe.transfers.create()` payout to the owner's connected account — after which owner completion and reviews from both parties worked with zero code paths specific to auctions, confirming the interest-pipeline-reuse design. Also verified the no-bids path closes to `'closed'` and correctly blocks completion. A concurrency test (two simultaneous equal-amount bids) confirmed exactly one wins and the audit log has exactly one row at that price.
  - **One real bug found and fixed during verification**: `placeBid`'s raw-SQL `WHERE` clause combined an `AND` chain with an unparenthesized `OR` — SQL's operator precedence let `current_bid_cents < amount` alone satisfy the entire clause, so a bid arbitrarily higher than the current price could win **even on an already-closed auction**, bypassing both the `status='open'` and deadline checks. Caught by a bid that landed on a `pending` auction during this exact verification pass. Fixed by wrapping the whole `OR` expression in one more pair of parens.
...
### Stage 8 — Location & proximity enforcement — ✅ Built
- **The gap this closes**: nothing stopped someone in Seoul from expressing interest in, booking, or bidding on a physical item listed by someone in New York — none of the transactional flows had any concept of where either party actually was.
- New location columns (`locationLat`/`locationLng`/`locationCity`/`locationRegion`/`locationCountry`/`locationCountryCode`) on both `users` (a profile "home base", set via a new Settings → Location screen) and `listings` (defaults to the owner's profile location at creation, overridable — the item may be at a storage unit, etc.). Only the 5 physical-handoff types (give_away/exchange/trial/rental/auction) require a resolved location; idea/lesson are exempt, since they have no transaction/completion state (§3).
- **Enforcement is distance-based, not country-based** — deliberately. A hard country check would incorrectly block a legitimate near-border meetup (e.g. two cities 5km apart but across a border) while a distance radius correctly and elegantly blocks the actually-absurd case (continent-to-continent) without political-border edge cases. Default max: **75km** (`MAX_TRANSACTION_DISTANCE_KM` env var), checked before `POST /:id/interest`, `POST /:id/bookings`, and `POST /:id/bids` all go through — missing location on either side blocks with a clear "set your location" message rather than silently passing.
- Geocoding (GPS coordinates ↔ city/country) is via **Nominatim (OpenStreetMap)**, free, no API key — consistent with this app's zero-recurring-cost ethos already established for Stage 5's background job. This carries a real obligation, not glossed over: a server-wide outbound throttle (max 1 request/second, independent of the existing per-client inbound rate limiter, which does nothing to cap *our* outbound calls), an in-memory cache keyed by rounded coordinates (a listing's location rarely changes), a real `User-Agent` header, and geocoding treated as best-effort enrichment — if Nominatim is down, the raw coordinates the user just captured are still saved, only the human-readable city/country is left blank.
- Location capture: new `components/LocationField.tsx`/`.web.tsx` pair (native via `expo-location`'s permission+GPS flow, web via the browser's built-in `navigator.geolocation`, both with a manual "type a city" fallback for denied permission), reused in both the new Settings screen and `app/listings/create.tsx` for the 5 physical types.
- Privacy: raw coordinates are never exposed to anyone but the location's own owner (needed only so their own edit UI can re-populate the picker) — every other viewer gets just `{city, region, country, countryCode}` plus a computed `distanceKm`, never the other party's coordinates. Browse results can sort by distance ("Near me" chip on the feed); a listing with no location sorts last rather than erroring or vanishing.
- **✅ Verified end-to-end against the deployed production API**: the USA↔Korea case is correctly blocked (~11,048km > 75km max) for interest, rental booking, and auction bidding alike; a user with no location set is blocked with a clear message rather than silently allowed; a nearby user (3km away) succeeds and sees a computed `distanceKm` with zero raw coordinates in the response; listing creation is correctly blocked for physical types without a resolved location and unaffected for `idea`; a listing with no explicit location correctly inherits the owner's profile location; the Nominatim reverse/search endpoints work live, with the in-memory cache confirmed via a measurable speedup on a repeated identical lookup.
- **Bug found and fixed during verification**: Nominatim returned place names in the local script (e.g. Korean for a Seoul search) rather than English, since no language hint was sent — the app's UI is English-only. Fixed by adding `accept-language=en` to every Nominatim request in `server/src/utils/geo.ts`. Reverified live post-deploy: `Seoul` now returns `{"city":"Seoul","country":"South Korea","countryCode":"KR"}`.
